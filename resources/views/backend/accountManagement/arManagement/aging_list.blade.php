		...
		<div class="page-wrapper">
			<div class="page-content">
                ...
                <ul class="nav nav-tabs">
                    <li class="pending_approval"><a href="{{url('/admin/ar/shipment-list')}}" data-toggle="tab" aria-expanded="true">Ar Dashboard</a>
                    </li>
        
                    <li class="all_leave"><a href="{{url('/admin/ar/payment-history')}}" data-toggle="tab" aria-expanded="false">Invoice Generated</a>
                    </li>
                    <li class="active"><a href="{{url('/admin/ar/all-payment')}}" data-toggle="tab" aria-expanded="false">All Invoice aging</a>
                        </li>
                </ul>
				<div class="card">
					<div class="card-body">
						<div class="table-responsive">
							<table class="table mb-0" id="carrier_table">
								...
								$i = 1;
								@endphp
                                    @foreach($Invoicedata as $shipments)
                                    @foreach($shipments as $shipment)
                                    @foreach($shipment->invoiceget as $invoice)
								    ...
								    <tr>
                						...
                						<td>{{ {{ isset($company['company_name']) ?  $company['company_name'] : Null }} }}</td>
                                        @endforeach
                						
                                        @foreach($invoice->priceget as $price)
                						<td>{{ {{ isset($price['customer_total']) ? $price['customer_total']  : Null }} }}</td>
                                        @endforeach

                						
                                        <td>{{ $start_date  = date('d M, Y', strtotime($invoice['start_date'])); }}</td>
                                        
                                        <?php
                                            
											$today = date('Y-m-d');
                                            $datetime1 = new DateTime($invoice['start_date']);
                                        ...
                                        @endforeach
                						<td>{{ $shipment['shipment_statue'] }}</td>
