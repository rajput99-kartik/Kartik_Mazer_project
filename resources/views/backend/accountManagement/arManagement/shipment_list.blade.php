						...
						<div class="table-responsive3">
							<table class="table mb-0" id="carrier_table">
								...
								@foreach($userdata as $user_result)
                                    @php
                                        $shipdataResult = App\Models\Shipment::where('user_id',$user_result->assign_agent_to)->where('ar_access_status','1')->where('ar_invoice', null)->orderBy('id','desc')->get();
								    @endphp
								@foreach($shipdataResult as $shipmentdata)
										...
										<td>
											<?php
                                            if(!empty($shipmentdata['ar_access_date'])){
