						...
						<ul class="navbar-nav align-items-center" id="clock_in_out">
						    ...
						    <!--</li>-->
                                    ...
                                    $cur_date = Carbon::createFromFormat('Y-m-d H:i:s', $datetime)->format('Y-m-d');
    									...
    									->where('date_data', $cur_date)->where('clockin', 1)->first();
    								?>
									...
									<form action="{{url('admin/clockin')}}" method="post">
										...
										<input type="hidden" name="timeout" value="<?php echo $cur_time; ?>">
								...
								@if($clockin->brackout == 0)
									<form action="{{url('admin/brackin')}}" method="post">
                                        ...
                                        $datetime = Carbon::now()->toDateTimeString();
                                        $cur_time = Carbon::createFromFormat('Y-m-d H:i:s', $datetime)->format('h:i:s A');
                                        $cur_date = Carbon::createFromFormat('Y-m-d H:i:s', $datetime)->format('Y-m-d h:i:s');
										
											$ddate = date("Y-n-j ") . date("H:i:s"); 
										?>

										<input type="hidden" name="brackin" value="<?php echo $cur_date; ?>">
								...
								@else
									...
									<form action="{{url('admin/brackout')}}" method="post">
										...
										<input type="hidden" name="clockin_id" value="<?php echo $clockin->id ; ?>">
										<input type="hidden" name="user_id" value="<?php echo Auth::id();?>">
										<input type="hidden" name="brackout" value="<?php echo $cur_date; ?>">
								...
								<form action="{{url('admin/clockin')}}" method="post">
									...
									<input type="hidden" name="timein" value="<?php echo $cur_time; ?>">
									...
									<div class="header-notifications-list">
										...
										@foreach ($notification_res as $notify_data )
										        ...
										        <?php 
        										    ...
        										    if(!empty($ul)){
										        ?>
                                                <div class="noti-list" status="{{$notify_data->status}}" id="notification_col" value="{{ $notify_data->id }}"><a href="javascript:void(0);"> <h6> <i class="bx bx-group"></i><span class="msg-time float-end" style="float: none !important;" id="notificationsRead" >{{ $notify_data->title }}</span></h6> </a> <span id="notificationsRead" value="{{ $notify_data->id }}"><?php if($notify_data->status == '0'){ ?><i class="bx bx-check"></i><?php } ?></span></div>
