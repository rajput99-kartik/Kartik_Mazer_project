...
// 				method:'POST',
// 				data:{companies_id:companies_id,status:status,comment:comment,limit:limit,prepay:prepay,titlecompany:titlecompany},
// 				cache : false,
// 				success:function(data){
...
$(document).on('click', '.request_update', function(e) {
    ...
    if (comment.trim() === '' || limit.trim() === '' && prepay.trim() === '') {
...
$(document).on('click','#EmailSubmitBtn', function(){
	...
	$.ajax({
		...
		success: function(data){
			setTimeout(function () {
				window.location.href = HOSTPATH+'admin/ar/shipment-detail/'+company_id;
			}, 500);
