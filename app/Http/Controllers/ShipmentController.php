...
class ShipmentController extends Controller
{
        ...
        function shipmentFilter(Request $request){
        ...
        public function createshipmentsubmit(Request $request){  
            ...
            abort(403, 'Sorry !! You are Unauthorized to view any admin !');
                ...
                $Shipment_dropId = $Shipment_drop->id;
                    ...
                    $carrier_total = ($request->input('rate_unit1_carrier')) + ($request->input('lh_carrier')) + ($request->input('carrier1')) + ($request->input('carrier2')) + ($request->input('carrier3')) + ($request->input('carrier4')) + ($request->input('carrier5'));

                    

                    $customer_total = ($request->input('unit1_customer')) + ($request->input('lh_customer')) + ($request->input('customer1')) + ($request->input('customer2')) + ($request->input('customer3')) + ($request->input('customer4')) + ($request->input('customer5'));
        ...
        /* Broker shipment update Function Start Here */
            ...
            public function shipmentupdate(Request $request){
                ...
                $carrier_total = ($request->input('lh_carrier')) + ($request->input('carrier1')) + ($request->input('carrier2')) + ($request->input('carrier3')) + ($request->input('carrier4')) + ($request->input('carrier5')) + ($request->input('carrier6')) + ($request->input('carrier7')) + ($request->input('carrier8'));

                    

                        

                $customer_total =  ($request->input('lh_customer')) + ($request->input('customer1')) + ($request->input('customer2')) + ($request->input('customer3')) + ($request->input('customer4')) + ($request->input('customer5')) + ($request->input('customer6')) + ($request->input('customer7')) + ($request->input('customer8'));
	...
	/* Broker shipment mc detail get Function End Here */
...

        {
            ...
            $pdf = PDF::loadView('backend.shipmentManagement.shipment.newpdf', ['user'=>$user,'todaydate'=>$todaydate,'shipmentData'=>$shipmentData,'carriersData'=>$carriersData,'shipmentpick'=>$shipmentpick,'shipmentdrop'=>$shipmentdrop,'shipmentrate'=>$shipmentrate])->setPaper($customPaper);
