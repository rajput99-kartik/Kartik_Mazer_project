	...
	/*Load get origin data function End*/	
      ...
      public function ratehistory(){
    	...
    	public function NewLanePost(Request $request){
          ...
          $dat_details = DB::table('dat_login')->first(); // (1120)for single
              ...
              if(empty($dat_details->username_dat)){
                return redirect('/admin/loads')->with('error','Please create DAT user name & token');
              }
        ...
        if($results){  
              ...
              if(empty($data)){
                return redirect('/admin/loads')->with('error', 'Create User Token && Check Rate !!');
              }
              $data;
              // END RATEVEIW HISTORY HERE       
                ...
                if(isset($res->id)){
                  //dd($res->id);
                    $new_load = new Load;	
                    $new_load->agent_name = $uname->name;
                    $new_load->load_dat_id = $res->id;
                    $new_load->user_id = $user_id;
                    $new_load->ref_no = $ref_no;
                    $new_load->load_state_origin_id = $pick_cities->city_id;
      ...
      $tz = date_default_timezone_set('America/New_York');
        ...
        //   $dat_details = DB::table('dat_login')->where('user_id',$uaid)->first();
          ...
          // if shipment or load deleted by DAT Account then we can use it 
              //Start
        ...
        public function LoadPostRateView(Request $request){
          ...
          $equipment_type =   strtoupper($request->input('equipment_type'));
      ...
      $user = DB::table('users')->where([
        ...
        public function LoadReportslistData(Request $request){
            if (is_null($this->user) || !$this->user->can('loads-reports')) {
                abort(403, 'Sorry !! You are Unauthorized to edit any user !');
                  ...
                  //   $results = Load::sortBy('created_at', 'desc')->get();
                  // $user_loads = Load::all();
                  $user_loads = Load::orderBy('id', 'DESC')->paginate('10');
              $page = 'Load';		
              return view('backend.loadsManagement.oldreportlist', compact('page','equipments','user_loads','userid', 'user_id','search',
            ...
            if(!empty($posts)){
                ...
                foreach ($posts as $post){
                    ...
                    $nestedData['options'] = '<td style="width: 80px;" class="action_tooltip classToInclude">
                        <input type="hidden" value="'.$show.'" id="load_dat_id">
                        <button type="button" class="btn btn-outline-secondary btn-sm radius-30 px-4" id="load_update_form_btn">
                        <i class="bx bx-edit"></i><span class="tooltip">Edit</span></button>
                    </td>';
                    $data[] = $nestedData;
                }
            }
        ...
        // public function LoadPostMultipalDeleted(Request $request, $id){
        //   $id = $request->id;
        //       // dd($id );
        //   foreach ($id as $lid) {
        //     Load::where('load_dat_id', $lid)->delete();
        //   }
        //   return redirect()->back(url('admin/loads'))->with('success','Load deleted successfully');
        // }


        
    public function loadmuldel12(Request $request)
    {
        $ids = $request->ids;
        // $datIds = $request->input('datId');
        $ids = explode(',',$ids);
        $updateProduct = Load::whereIn('load_dat_id',$ids)->update(['load_status' => '1']);
        return response()->json(['status'=>true,'message'=>"Data successfully removed."]);
         
    }


        public function loadmuldel(Request $request){
          $datIds = $request->input('datId');
          // dd($datIds);
          foreach ($datIds  as $datId) {
              $user_id = auth()->user()->id;
                  $user = DB::table('users')->where([	['id', '=', $user_id], ])->first();
                    $aid = Auth::id();
                    $mid = User::where('id', $aid)->pluck('dat_user_auth_status')->first();
              ...
              if($error_msg == ''){
                ...
                //  return redirect()->back()->with('success','Load deleted successfully-1');
                // continue;
          }
          return redirect()->back()->with('success','Load deleted successfully-1');          
        }
